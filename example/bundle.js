...
function isStrictlyRight(p) {
        return orientation_1(this.a, this.b, p) < 0 ? 1 : 0
}
...
function removeColinearPoints(S) {
    ...
    for (var i=0; i < S.length; i++) {
        if ( orientation_1(S[(i+l-1)%l], S[i], S[(i+1)%l]) != 0 ) {
            Sclean.push(S[i]);
        }
    }
...
function quickHullInner(S, a, b) {
    ...
    var d = S.map(function(p) {return {dist: orientation_1(a,b,p)*distLineToPoint(a, b, p), point: p}});
...
var toColor_1 = toColor;

var kdtree = createCommonjsModule(function (module, exports) {
/**
 * kd-tree-js v0.0.1
 * kd-tree
 *
...
var map = commonjsGlobal.map = new L.Map('map', {})
  ...
  L.Util.requestAnimFrame(function() {
    for (var i = 0; i < numMarkers; i++) {
      ...
      markers[i]
        ...
        .setStyle({
          color: toColor_1(nearestPoint.feature.properties.city)
        });
